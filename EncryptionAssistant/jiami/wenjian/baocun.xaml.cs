...
namespace EncryptionAssistant.jiami.wenjian
{
    ...
    public sealed partial class baocun : Page
    {
        ...
        {
            if(App.Huancun.jiami_wenjian.baocun_dizhi!=null)
            {
                dizhi_shuru.Text = App.Huancun.jiami_wenjian.baocun_dizhi.Path;
            }
            gaibiandaxiao((int)(ActualWidth - 100) / 4, 60);

            SizeChanged += Baocun_SizeChanged;
            //记录
            App.Huancun.jiami_wenjian.yeshu = 3;
        }
        ...
        private void xiayibu_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                var resourceLoader = Windows.ApplicationModel.Resources.ResourceLoader.GetForCurrentView("jia_baocun");
                //"请选择一个文件夹以保存加密结果"
        ...
        private async void liulan_ClickAsync(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                if (folder != null)
                {
                    ...
                    //添加我
                    App.Huancun.jiami_wenjian.baocun_dizhi = folder;
